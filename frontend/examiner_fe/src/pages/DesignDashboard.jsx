...
import {
  ...
  FileText, CheckCircle, Hash, Building2, AlertCircle, Zap, Hourglass
} from 'lucide-react';
// API 함수는 기존 코드를 그대로 유지합니다.
import { getDashboard, getReviewList } from '../api/review';
...
// [수정] API 상태 코드와 UI 표시 정보를 매핑하는 객체 (APPROVE, REJECT로 수정)
const statusMap = {
  ...
  'APPROVE': { label: '심사완료', color: 'bg-green-100 text-green-700' },
...
export default function DesignDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      const userId = '201'; // 실제 구현 시에는 로그인 상태에서 가져와야 합니다.

      try {
        setLoading(true);
        const listResponse = await getReviewList(userId);

        // [수정] getReviewList 응답을 기반으로 대시보드 통계를 직접 계산합니다. (APPROVE, REJECT로 수정)
        setDashboardStats({
        ...
        setDesignData(listResponse.map(item => {
          ...
          return {
            ...
            description: item.description || '디자인에 대한 구체적인 요약 정보가 없습니다.',
            field: item.field || '미정',
  ...
  // [수정] 필터링 로직을 API 상태 코드에 맞게 수정합니다. (APPROVE, REJECT로 수정)
        ...
        <div className="space-y-4">
          {filteredData.length > 0 ? (
            filteredData.map((item) => {
              // [수정] 진행 상황 로직을 API 상태 코드에 맞게 수정했습니다. (APPROVE, REJECT로 수정)
              const getStageStatus = (stageId) => {
                switch (stageId) {
                  case 'reception':
                    return { completed: true, current: false };
                  case 'waiting':
                    return { completed: ['REVIEWING', 'APPROVE', 'REJECT'].includes(item.status), current: item.status === 'SUBMITTED' };
                  case 'examination':
                    return { completed: ['APPROVE', 'REJECT'].includes(item.status), current: item.status === 'REVIEWING' };
                  case 'decision':
                    return { completed: item.status === 'APPROVE', current: ['APPROVE', 'REJECT'].includes(item.status) };
                  case 'registration':
                    return { completed: item.status === 'APPROVE', current: item.status === 'APPROVE' };
                  default:
